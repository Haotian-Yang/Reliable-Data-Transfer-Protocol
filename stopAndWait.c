...
// int for time out
int timeout;
// keep track of sequence on A side
int Aseq;
// keep track of msg for resend
struct msg copy;
...
{
    // creste new struct
    printf("[A] sending Message to B: %s, currentACK: %d\n", message.data, Aseq);
    int cksum = (int)checksum(Aseq, Aseq, message.data);
    struct pkt pkt2give;
    pkt2give.acknum = Aseq;
    pkt2give.seqnum = Aseq;
    pkt2give.checksum = cksum;
    strncpy(pkt2give.payload, message.data, MSG_SIZE);
    strncpy(copy.data, message.data, MSG_SIZE);

    // call to net work layer
    to_network_layer(0, pkt2give);
    // start time out
    starttimer(0, timeout);
}

/* function to calculate the checksum 
 * Input: sequnence number, ACK , and payload
 * Output: unsigned int 
 */
unsigned int checksum(int seq, int ack, char *payload)
{
    unsigned int result = 0;
    // accumulate all the char in the payload
...
{
    // use checksum to check corruption
    printf("[A] Receiving ACK from B. ACK=%d", packet.acknum);
    int cksum_real = checksum(Aseq, Aseq, "");
    int cksum_rec = checksum(packet.seqnum, packet.acknum, packet.payload);
    // not corrupted AND check if reciving right ack
    if (cksum_real == cksum_rec && Aseq == packet.acknum)
    {
        // stop timer
...
void A_timeout(void)
{
    printf("[A] ---- TIMEOUT ---- currentACK=%d\n", Aseq);
    // resend the msg
    A_send(copy);
}

// keep track of sequence on B side
int BSeq;
...
void B_recv(struct pkt packet)
{
    ...
    // calculate the checksum based on the packet recevied
    int checkCheckSum = (int)checksum(packet.seqnum, packet.acknum, packet.payload);
    // check if corruption
    // Corruption
    if (packet.checksum != checkCheckSum)
    {
        printf("[B] ----- CHECKSUM MISMATCH (Corrupted Packet) -----\n");
    }
    // No Corruption
    else
    {
        // check if we receiving a duplicated seq
        if (packet.seqnum == BSeq)
        {
            printf("[B] ----- Received a DUP Packet ----- packetAckNum=%d\n", packet.acknum);
        }
        // not duplicated seq
        else
        {
            //Sending Data to the packet number
            to_application_layer(1, packet.payload);
            BSeq ^= 1;
            printf("[B] ----- RECEIVING GOOD PACKET ----- packetAckNum=%d BSeq=%d\n", packet.acknum, BSeq);
        }
    }
